...
async def create_order_endpoint(order: OrderCreate, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    """
    logger.info(f"Creating order for user: {user.phone_number} (Admin: {user.is_admin})")
...
async def list_orders(db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    """
    if user.is_admin:
        logger.info(f"Admin {user.phone_number} listing all orders")
        orders, total = await get_orders(db)
        return orders
    else:
        logger.info(f"User {user.phone_number} listing their own orders")
...
async def update_order_endpoint(order_id: int, order_update: OrderUpdate, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    if not user.is_admin and db_order.user_id != user.id:
        logger.warning(f"User {user.phone_number} (Admin: {user.is_admin}) attempted to update order {order_id} belonging to user {db_order.user_id}")
        raise HTTPException(
            status_code=403, 
            detail="You can only update your own orders"
        )
    
    logger.info(f"Updating order {order_id} by user: {user.phone_number} (Admin: {user.is_admin})")
...
async def delete_order_endpoint(order_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    # Check permissions
    if not user.is_admin and db_order.user_id != user.id:
        logger.warning(f"User {user.phone_number} (Admin: {user.is_admin}) attempted to delete order {order_id} belonging to user {db_order.user_id}")
        raise HTTPException(
            status_code=403, 
            detail="You can only delete your own orders"
        )
    
    logger.info(f"Deleting order {order_id} by user: {user.phone_number} (Admin: {user.is_admin})")
